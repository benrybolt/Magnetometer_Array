...
// below to // stop should output the time see //stop in main() as well
#include <stdio.h>
#include <time.h>
//stop
...
int main(int argc, char *argv[])
{
  ...
  while(1) {
    ...
    case 'r':
      ...
      printf("%s    %s", buf, ctime(&curtime) );
      //printf("Current time = %s\n",ctime(&curtime) );//time
      //      printf("%s",ctime(&curtime) );//time
      break;
    case 'F':
      if( fd == -1 ) error("serial port not opened");
      ...
      serialport_flush(fd);
      break;
  //print time with lines below end at //stop
